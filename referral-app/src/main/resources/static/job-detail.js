document.addEventListener("DOMContentLoaded", async () => {
  ...
  const alumni = alumniList.find((item) => item.id === job.alumniId);
  const company = companyList.find((item) => item.id === job.companyId);
  const similarJobs = jobs.filter((item) => item.id !== job.id && item.city === job.city).slice(0, 3);
  await fetchFavoriteJobIds(session.profileId);
  const favorite = isFavoriteJob(session.profileId, job.id);

  renderAppLayout("jobs", "岗位详情", "查看岗位要求、发布校友和企业信息。", `
    <section class="detail-hero">
      <div class="panel reveal">
        <div class="section-eyebrow">Job Profile</div>
        ...
        <div class="detail-actions">
          ...
          <button class="btn" id="go-apply">立即投递</button>
          <button class="btn secondary-btn" id="go-consult">咨询校友</button>
        </div>
      </div>
      <div class="detail-side">
        <div class="panel floating-panel reveal reveal-delay-1">
          <div class="section-eyebrow">Referral Flow</div>
          <div class="panel-header"><div><h2>投递节奏</h2><p>答辩时可以直接解释这条业务闭环。</p></div></div>
          <div class="stepper">
        ...
        <div class="panel reveal reveal-delay-2">
          <div class="section-eyebrow">Key Notes</div>
          <div class="compact-list">
            <div class="compact-item"><strong>联系形式</strong><div class="job-card-company">${job.contactType || "站内沟通"}</div></div>
            <div class="compact-item"><strong>经验要求</strong><div class="job-card-company">${job.experienceRequirement || "有项目经验优先"}</div></div>
            <div class="compact-item"><strong>过期时间</strong><div class="job-card-company">${formatDateTime(job.expireTime) || "长期有效"}</div></div>
          </div>
        </div>
      </div>
    </section>
    <section class="grid-2">
      <div class="panel reveal reveal-delay-1">
        <div class="panel-header"><div><h2>对接校友</h2><p>岗位发布人和沟通对象。</p></div></div>
      ...
      <div class="panel reveal reveal-delay-2">
        <div class="panel-header"><div><h2>企业信息</h2><p>便于求职者快速了解公司背景。</p></div></div>
        <div class="compact-item">
    ...
    <section class="panel reveal reveal-delay-3">
      <div class="panel-header"><div><h2>相似岗位</h2><p>同城市岗位推荐，适合继续浏览。</p></div></div>
